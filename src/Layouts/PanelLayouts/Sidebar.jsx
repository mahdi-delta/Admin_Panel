...
const Sidebar = ({ SidebarActive, ActivePage, setSidebarActive, setActivePage }) => {
     ...
     return (
          <>
               ...
               {SidebarActive && (
                    <div
                         className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[9] hidden max-mid:block"
                         onClick={() => setSidebarActive(false)}
                    />
               )}

               <section
                    className={`p-1 bg-transparent transition-all duration-300 text-white overflow-y-auto max-mid:fixed max-mid:top-0 max-mid:left-0 max-mid:h-screen z-10 ${
                         SidebarActive ? "w-70 max-mid:w-60" : "w-20 max-mid:-translate-x-full"
                    }`}
               >
                    <div
                         className={`w-full h-max rounded-xl bg-midnight ${
                              SidebarActive
                                   ? "p-4 pb-4 border-slate max-mid:border-r-2 max-mid:rounded-none max-mid:rounded-r-xl"
                                   : "p-3 pb-0"
                         }`}
                    >
                         <div className="max-w-80 flex flex-col gap-6 justify-center items-center">
                              ...
                              >
                                   <div
                                        style={{
                                             scale: !SidebarActive && 0,
                                             opacity: !SidebarActive && 0,
                                        }}
                                        className="transition-[scale,opacity] duration-200"
                                   >
